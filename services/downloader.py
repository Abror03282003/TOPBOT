...
# Ayni vaqtda faol va ishlayotgan ommaviy API instansiyalari
COBALT_INSTANCES = [
...
# ---------------------------------------------------------------------------
# QIDIRUV (Piped -> Invidious -> YouTube Web Scraping)
# ---------------------------------------------------------------------------
async def search_tracks(query: str, limit: int = 30) -> list[dict]:
    ...
    # 2. Piped orqali qidiruv
    results = await _search_via_piped(search_query, limit)
    if results:
        return results

    # 3. Zaxira: yt-dlp flat extraction
    def _yt_flat():
        opts = {
            'extract_flat': True,
            'skip_download': True,
...
# ---------------------------------------------------------------------------
# AUDIO YUKLASH (Cobalt API -> Invidious Direct -> yt-dlp + Cookies)
# ---------------------------------------------------------------------------
async def download_audio_by_id(video_id_or_url: str) -> tuple[str | None, str, str | None]:
    ...
    # 1. Cobalt API bilan yuklab olish (IP blokirovkalarni to'liq aylanib o'tadi)
    cobalt_file = await _download_via_cobalt(url, file_prefix)
    if cobalt_file:
        return cobalt_file, "Audio Track", None

    # 2. Invidious Direct Stream bilan yuklab olish
                            ...
                            async with session.get(media_link, timeout=aiohttp.ClientTimeout(total=40)) as file_resp:
                                if file_resp.status == 200:
                                    ...
                                    if os.path.exists(output_path) and os.path.getsize(output_path) > 10240:
                                        logging.info(f"✅ Cobalt API orqali muvaffaqiyatli yuklandi.")
                                        return output_path
            except Exception as e:
                logging.warning(f"Cobalt error ({instance}): {e}")
                continue
    return None
                        ...
                        if s_resp.status == 200:
                            with open(output_path, "wb") as f:
                                async for chunk in s_resp.content.iter_chunked(64 * 1024):
                                    f.write(chunk)
                            if os.path.exists(output_path) and os.path.getsize(output_path) > 10240:
                                logging.info(f"✅ Invidious orqali yuklandi.")
...
def _yt_dlp_download_audio(url: str, file_prefix: str) -> tuple[str | None, str]:
    ...
    try:
        ...
        logging.error(f"yt-dlp oxirgi chora ham amalga oshmadi: {e}")
