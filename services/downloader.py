...
# Boshqa fayllardan import qilinadigan funksiyalarni aniq ko'rsatish
...
CLIENT_ATTEMPTS = [
    (['mweb', 'android'], True),
    (['ios', 'web_creator'], True),
    (['tv', 'tv_embedded'], False),
    (['android_vr'], False),
    (None, False),
]
        ...
        for instance in PIPED_INSTANCES:
            try:
                ...
                async with session.get(url, timeout=aiohttp.ClientTimeout(total=6)) as resp:
                    ...
                    async with session.get(audio_url, timeout=aiohttp.ClientTimeout(total=35)) as stream_resp:
                ...
                logging.warning(f"Piped error ({instance}): {e}")
                continue
